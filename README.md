...
This project enhances the [CDAP Wrangler]library by adding support for parsing and aggregating byte size and time duration values natively in recipes.

## 🚀 New Features

### ✅ 1. Byte Size Token

You can now use values like 512KB, 1.5MB, 2GB, etc., directly in Wrangler recipes.

#### Supported Units:
- B (Bytes)
- KB, MB, GB, TB (both lowercase and uppercase)
...
### ✅ 2. Time Duration Token

Wrangler now also understands time durations like 500ms, 1.5s, 2min, etc.
...
This directive performs aggregation (summing) of ByteSize and TimeDuration columns.

#### 🔧 Syntax:
aggregate-stats :<byte_column> :<duration_column> <output_byte_column> <output_time_column>
...
|---------------------|-------------------------------------------------|
| :<byte_column>     | Input column with byte size values (e.g., 1MB) |
| :<duration_column> | Input column with time values (e.g., 500ms)    |
| output_byte_column | Output column name for total MB                |
| output_time_column | Output column name for total seconds           |
